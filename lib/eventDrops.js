          ...
          zoomRect.on('mousemove', function(d, e) {
            ...
            if (!el || el.tagName !== 'circle') {
              if(el && el.tagName === 'svg') {
                var rpos = el.createSVGRect();
                var bounds = el.getBoundingClientRect();
                rpos.x = d3.event.clientX - (bounds.x || bounds.left);
                rpos.y = bounds.height - (d3.event.clientY - (bounds.y || bounds.top));
                rpos.width = rpos.height = 10;
                var list = el.getIntersectionList(rpos, null);
                for(var x = 0; x < list.length; ++x) {
                  if(list[x].tagName === 'circle') {
                    config.eventHoverComplete(selection[0][0]);
                    config.eventHover(d3.select(list[x]).datum(), list[x], selection[0][0]);
            ...
            zoomRect.attr('display', 'block');
            config.eventHoverComplete(selection[0][0]);
            config.eventHover(d3.select(el).datum(), el, selection[0][0]);
            return;
          });
        }
