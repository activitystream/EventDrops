          ...
          zoomRect.on('mousemove', function(d, e) {
            ...
            if (!el || el.tagName !== 'circle') {
              if(el && el.tagName === 'svg') {
                let rpos = el.createSVGRect();
                let bounds = el.getBoundingClientRect();
                console.log(bounds);
                console.log(el);
                rpos.x = d3.event.clientX - bounds.left;
                rpos.y = bounds.height - (d3.event.clientY - bounds.top);
                console.log('x: '+ rpos.x + ' y: ' + rpos.y);
