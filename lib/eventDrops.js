  ...
  return function eventDrops(config) {
    ...
    function eventDropGraph(selection) {
      selection.each(function (data) {
        ...
        var inhibit = false;
        function updateZoom(notify, p, translate, scale) {
          if(inhibit) {
              return;
          }
          if(notify) {
            var center = zoom.center();
            zoom.center(startCenter);
            zoom.translate(startTranslate);
            zoom.scale(startScale);
            config.zoomCallback(d3.event.translate, d3.event.scale, d3.mouse(zoomRect[0][0]));
            return;
          }
        ...
        function redraw() {
          ...
          lines.enter()
            ...
            .call(eventLine({ hasLabels: config.hasLabels, xScale: xScale, eventColor: config.eventColor }))
          ;
